...
<div class="mb-6"><a href="{{ route('drivers.index') }}" class="inline-flex items-center gap-1 text-sm text-slate-400 hover:text-amber-400 transition"><i data-lucide="arrow-left" class="w-4 h-4"></i>Drivers</a>
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mt-2">
  <div class="flex items-center gap-3"><h1 class="font-heading text-2xl sm:text-3xl font-extrabold uppercase tracking-tight">{{ $model->name }} <span class="mono text-slate-500 text-lg normal-case">{{ $model->code }}</span></h1><x-badge :status="$model->status"/></div>
  <a href="{{ route('drivers.edit',$model) }}" class="btn btn-primary" data-testid="edit-drivers-detail"><i data-lucide="pencil" class="w-4 h-4"></i>Edit</a>
</div></div>
